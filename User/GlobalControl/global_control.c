...
void Global_Init(void)
{
  ...
  LCD_Init();
	...
	__HAL_TIM_SET_COMPARE(&htim1,TIM_CHANNEL_3,MC.Foc.DutyCycleC); 		 // Stepper: B+
	__HAL_TIM_SET_COMPARE(&htim1,TIM_CHANNEL_4,MC.Foc.DutyCycleD);		               // Stepper: B-
...
{		
	...
	MC.Position.MechanicalPosSet = -ADC2->JDR4;//?ò?λ?λ?λ?????
	...
	__HAL_TIM_SET_COMPARE(&htim1,TIM_CHANNEL_2,MC.Foc.DutyCycleB);     // Stepper: A-
	__HAL_TIM_SET_COMPARE(&htim1,TIM_CHANNEL_3,MC.Foc.DutyCycleC); 		 // Stepper: B+
	__HAL_TIM_SET_COMPARE(&htim1,TIM_CHANNEL_4,MC.Foc.DutyCycleD);		               // Stepper: B-		               //更新PWM比较值
