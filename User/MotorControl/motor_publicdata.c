/**
  ******************************************************************************
	MC.Motor.RunMode = CURRENT_CLOSE_LOOP;          // Stepper demo: current loop
	MC.Encoder.Type = ENCODER_TYPE_OPEN_LOOP;			 // Stepper: open-loop theta generation
	MC.Encoder.ElectricalSpdSet = 200.0f;			 // Demo electrical speed (RPM)
	MC.IqPid.Ref = 0.5f;                          // Demo Iq reference (A)
	MC.IdPid.Ref = 0.0f;                          // Demo Id reference (A)
  * 作    者: 浩然
  * 版    本: V1.0
  * 编写日期: 
  * 功    能: 
