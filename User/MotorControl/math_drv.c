...
/**
  ...
  * 输入参数:
  * 返回参数:
  * 说    明:输入0-4095 对应0-360度 
  */
void Calculate_Sin_Cos(s32 angle,float *sinval,float *cosval)
{ 	
  Amplitude_Limit(&angle,0,4095);	
	angle = angle * 0.25f;  //0-4095转换为0-1023 与数组大小对应
  switch (angle & SIN_RAD)   
  {
    case U0_90:                        
       *sinval = Sin_Cos_Table[(uint8_t)(angle)];  
       *cosval = Sin_Cos_Table[(uint8_t)(0xFF-(u8)(angle))];
    break;
  
    case U90_180:  
       *sinval = Sin_Cos_Table[(u8)(0xFF-(u8)(angle))]; 
       *cosval = -Sin_Cos_Table[(u8)(angle)];
    break;
  
    case U180_270:
       *sinval = -Sin_Cos_Table[(u8)(angle)];
       *cosval = -Sin_Cos_Table[(u8)(0xFF-(u8)(angle))];
    break;
